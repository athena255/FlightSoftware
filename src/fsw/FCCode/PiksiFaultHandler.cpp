...
PiksiFaultHandler::PiksiFaultHandler(StateFieldRegistry& r) 
    ...
    {
        this->add_writable_field(no_cdgps_max_wait_f);
        this->add_writable_field(cdgps_delay_max_wait_f);
        this->add_writable_field(fault_handler_enabled_f);
        ...
        piksi_state_fp = this->find_readable_field<unsigned char>("piksi.state", __FILE__, __LINE__);
        mission_state_fp = this->find_writable_field<unsigned char>("pan.state", __FILE__, __LINE__);
        last_rtkfix_ccno_fp  = this->find_internal_field<unsigned int>("piksi.last_rtkfix_ccno", __FILE__, __LINE__);
        enter_close_appr_time_fp = this->find_internal_field<unsigned int>("pan.enter_close_approach_ccno", __FILE__, __LINE__);
    }
